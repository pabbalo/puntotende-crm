...
// Funzione per il download dei documenti attraverso AJAX
function puntotende_download_documento() {
    ...
    if (!wp_verify_nonce($nonce, 'download_documento_' . $documento_id)) {
    ...
    $documento = $wpdb->get_row($wpdb->prepare(
        "SELECT * FROM $table_documenti WHERE id = %d",
        $documento_id
    ));
    ...
    if (!file_exists($file_path)) {
        wp_die('File non trovato sul server.');
    }
    
    // Forza il download
    header('Content-Description: File Transfer');
    header('Content-Type: ' . $documento->tipo_file);
    header('Content-Disposition: attachment; filename=' . $documento->nome_file);
...
add_action('wp_ajax_puntotende_download_documento', 'puntotende_download_documento');
...
function puntotende_crm_clienti_edit() {
    ...
    require_once plugin_dir_path(__FILE__) . 'includes/tabs/appuntamenti.php';
    require_once plugin_dir_path(__FILE__) . 'includes/tabs/telefonate.php';
    require_once plugin_dir_path(__FILE__) . 'includes/tabs/documenti.php';
    ...
    <nav class="nav-tab-wrapper">
        ...
        <a href="<?php echo esc_url(admin_url('admin.php?page=puntotende-crm-clienti-edit&cliente_id=' . $cliente_id . '&tab=documenti')); ?>" 
           class="nav-tab <?php echo $current_tab === 'documenti' ? 'nav-tab-active' : ''; ?>">
           Documenti
        </a>
    </nav>
    ...
    switch ($current_tab) {
        ...
        case 'telefonate':
            ...
            break;
        case 'documenti':
            if (function_exists('puntotende_crm_show_documenti')) {
                puntotende_crm_show_documenti($cliente);
            } else {
                echo '<div class="error"><p>Funzionalità documenti non disponibile.</p></div>';
            }
            break;
        default:
