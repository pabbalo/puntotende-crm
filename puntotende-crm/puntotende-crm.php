...
// Funzione per creare la tabella puntotende_allegati
function puntotende_crm_create_tables() {
    global $wpdb;
    $table_allegati = $wpdb->prefix . 'puntotende_allegati';

    // Verifica se la tabella esiste già
    if ($wpdb->get_var("SHOW TABLES LIKE '$table_allegati'") != $table_allegati) {
        $charset_collate = $wpdb->get_charset_collate();

        $sql = "CREATE TABLE $table_allegati (
...
register_activation_hook(__FILE__, 'puntotende_crm_create_tables');
...
require_once plugin_dir_path(__FILE__) . 'includes/tabs/allegati.php'; // Nuovo tab

// Funzione per modificare il cliente
function puntotende_crm_clienti_edit() {
    ...
    if ($cliente_id <= 0) {
        wp_die('Cliente non trovato.');
    }

    $cliente = $wpdb->get_row($wpdb->prepare("SELECT * FROM $table_clienti WHERE id = %d", $cliente_id));
    if (!$cliente) {
        wp_die('Cliente non trovato.');
    }
    ...
    echo '<a href="?page=puntotende-crm-clienti&action=edit&cliente_id=' . esc_attr($cliente_id) . '&tab=info" class="nav-tab">Informazioni</a>';
    echo '<a href="?page=puntotende-crm-clienti&action=edit&cliente_id=' . esc_attr($cliente_id) . '&tab=appuntamenti" class="nav-tab">Appuntamenti</a>';
    echo '<a href="?page=puntotende-crm-clienti&action=edit&cliente_id=' . esc_attr($cliente_id) . '&tab=telefonate" class="nav-tab">Telefonate</a>';
    echo '<a href="?page=puntotende-crm-clienti&action=edit&cliente_id=' . esc_attr($cliente_id) . '&tab=allegati" class="nav-tab">Allegati</a>'; // Nuovo tab
    echo '</h2>';
    ...
    switch ($tab) {
        ...
        case 'telefonate':
            ...
            break;
        case 'allegati': // Nuovo tab
            puntotende_crm_show_allegati($cliente);
            break;
        case 'info':
        default:
...
// FINE Funzione per creare la tabella puntotende_allegati
...
require_once plugin_dir_path(__FILE__) . 'includes/class-mobile-api.php';

/**
 * Registra route per accedere alla webapp mobile
 */
function ptcrm_register_webapp_route() {
    add_rewrite_rule(
        'ptcrm-mobile/?$',
        'index.php?ptcrm_mobile=true',
        'top'
    );
    
    add_rewrite_tag('%ptcrm_mobile%', '([^&]+)');
}
add_action('init', 'ptcrm_register_webapp_route');

/**
 * Carica la webapp quando viene richiesta
 */
function ptcrm_load_webapp() {
    if (get_query_var('ptcrm_mobile') === 'true') {
        include(plugin_dir_path(__FILE__) . 'mobile-app/index.html');
        exit;
    }
}
add_action('template_redirect', 'ptcrm_load_webapp');

/**
 * Aggiungi link alla webapp nel menu admin
 */
function ptcrm_add_webapp_link($links) {
    // Aggiungi nel footer del plugin
    echo '<p><a href="' . home_url('ptcrm-mobile') . '" target="_blank" class="button">Versione Mobile</a></p>';
}
add_action('ptcrm_admin_footer', 'ptcrm_add_webapp_link');

// AGGIUNTI DOPO// Nel file principale del plugin PuntoTende CRM
function ptcrm_register_basic_api_endpoints() {
    ...
    register_rest_route('ptcrm/v1', '/dashboard', [
        'methods' => 'GET',
        'callback' => 'ptcrm_api_dashboard',
        'permission_callback' => function() {
            return current_user_can('read');
        }
    ]);
}
add_action('rest_api_init', 'ptcrm_register_basic_api_endpoints');

// Endpoint per stato autenticazione
...
// Endpoint per dashboard (dati demo)
function ptcrm_api_dashboard() {
    return [
        'stats' => [
...
function puntotende_crm_clienti_edit() {
    ...
    require_once plugin_dir_path(__FILE__) . 'includes/tabs/appuntamenti.php';
    require_once plugin_dir_path(__FILE__) . 'includes/tabs/telefonate.php';
    require_once plugin_dir_path(__FILE__) . 'includes/tabs/allegati.php'; 
