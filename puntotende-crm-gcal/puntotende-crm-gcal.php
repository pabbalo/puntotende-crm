...
add_filter('puntotende_gcal_update_event', function($result, $gcal_event_id, $event_data) {
    try {
        $gcal = PuntoTende_GCal::get_instance();
        ...
        if (!method_exists($gcal, 'update_event')) {
            throw new Exception('Il metodo update_event non esiste nella classe PuntoTende_GCal');
        }
        
        // Assicurati che l'ID dell'evento sia incluso nei dati
...
add_filter('puntotende_gcal_delete_event', function($result, $event_id) {
    try {
        ...
        if (!method_exists($gcal, 'delete_event')) {
            throw new Exception('Il metodo delete_event non esiste nella classe PuntoTende_GCal');
        }
